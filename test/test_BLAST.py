...
for _ in range(47):
    ...
    for __ in range(_+1,47):
        ...
        past = time.time()
        b = conventional.BLAST(True,seq1,seq2,[seq[_],seq[__]],"result/BLAST/result%04d%02d%02d%02d%02d%02d" % (
        startdate.tm_year, startdate.tm_mon, startdate.tm_mday, startdate.tm_hour, startdate.tm_min, startdate.tm_sec))
        b.align()
        #coords1, coords2, aligns1, aligns2, score = m.export_info()
        now = time.time()
        ...
        print("test",_,__)
        print("time", str(now-past)+"s", str(now-start)+"s")
    
        filename = "result/BLAST/result%04d%02d%02d%02d%02d%02d.txt" % (
            startdate.tm_year, startdate.tm_mon, startdate.tm_mday, startdate.tm_hour, startdate.tm_min,
            startdate.tm_sec)

        file = open(filename,"a")
        file.write(str(_)+" "+str(__)+" "+str(now-past)+" "+str(now-start)+"\n")
        file.close()

        if FLAGS.print_align:
            filename = "align/BLAST/result%04d%02d%02d%02d%02d%02d" % (
                startdate.tm_year, startdate.tm_mon, startdate.tm_mday, startdate.tm_hour, startdate.tm_min,
                startdate.tm_sec)
            b.print(filename+"_"+str(_)+"_"+str(__))
        
'''
        if FLAGS.show_align:
            filename = "img/BLAST/result%04d%02d%02d%02d%02d%02d" % (
                startdate.tm_year, startdate.tm_mon, startdate.tm_mday, startdate.tm_hour, startdate.tm_min,
                startdate.tm_sec)
